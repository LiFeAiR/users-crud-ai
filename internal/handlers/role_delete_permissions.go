...
func (bh *BaseHandler) DeleteRolePermissions(
	...
	if err := bh.roleRepo.DeleteRolePermissions(ctx, int(in.RoleId), convertInt32SliceToInt(in.PermissionIds)); err != nil {
	...
	role, err := bh.roleRepo.GetRoleWithPermissions(ctx, int(in.RoleId))
	if err != nil {
