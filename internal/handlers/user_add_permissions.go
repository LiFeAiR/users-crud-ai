...
func (bh *BaseHandler) AddUserPermissions(
	ctx context.Context,
	...
	if err := bh.userRepo.AddUserPermissions(ctx, int(in.UserId), convertInt32SliceToInt(in.PermissionIds)); err != nil {
		log.Printf("add user permissions failed, err:%v\n", err)
		return nil, status.Error(codes.Internal, "Failed to add user permissions")
	}
